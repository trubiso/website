<script lang="ts">
  const ss = (a: string) =>
    document.getElementsByTagName('section')
      ? (document.getElementsByTagName('section')[0].innerHTML = a)
      : 1;
  const cln = () => {
    ss(
  ...
  const lp = () => {
    document.getElementById('huh').parentElement.innerHTML +=
      "<br><img src='https://cdn.discordapp.com/emojis/717683480787550228.png' alt='h' />";
    setTimeout(lp, 1000);
  };
  const nl = () => {
    if ((document.getElementById('huh') as HTMLInputElement)?.value === 'dQw4w9WgXcQ') lp();
  };
</script>
